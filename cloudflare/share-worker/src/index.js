...
function htmlResponse(request,id,p){const host=new URL(request.url).origin,share=`${host}/?id=${encodeURIComponent(id)}`,image=p.imageUrl||`${host}/image?id=${encodeURIComponent(id)}`,target=targetUrl(id),title=clean(p.title||"Berita SMP Negeri 1 Susukan",220),desc=clean(p.description||p.summary||"Kabar terbaru SMP Negeri 1 Susukan",320);const html=`<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} — ${SCHOOL}</title><meta name="description" content="${esc(desc)}"><meta property="og:type" content="article"><meta property="og:locale" content="id_ID"><meta property="og:site_name" content="PAIBP SMART SMP"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(desc)}"><meta property="og:url" content="${esc(share)}"><meta property="og:image" content="${esc(image)}"><meta property="og:image:alt" content="Sampul ${esc(title)}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${esc(title)}"><meta name="twitter:description" content="${esc(desc)}"><meta name="twitter:image" content="${esc(image)}"><link rel="canonical" href="${esc(share)}"></head><body><p>${esc(title)}</p></body></html>`;return new Response(html,{headers:{"Content-Type":"text/html;charset=utf-8","Cache-Control":"public,max-age=30,s-maxage=60","X-Robots-Tag":"index,follow"}})}

export default{async fetch(request){const u=new URL(request.url),id=clean(u.searchParams.get("id"),160),origin=request.headers.get("Origin")||"";if(request.method==="OPTIONS")return new Response(null,{status:204,headers:cors(origin)});if(u.pathname==="/health")return new Response(JSON.stringify({ok:true,service:"paibp-smart-share",version:"120"}),{headers:{"Content-Type":"application/json","Cache-Control":"no-store"}});if(u.pathname==="/register"&&request.method==="POST")return register(request);if(!id)return Response.redirect(`${SITE}#kabar-spensus`,302);if(u.pathname==="/image")return imageResponse(request,id);
 // Browser manusia langsung diarahkan dengan HTTP redirect. Tidak bergantung JavaScript/meta-refresh.
 if(!isPreviewCrawler(request))return Response.redirect(targetUrl(id),302);
 // Bot preview tetap menerima Open Graph judul + sampul posting.
