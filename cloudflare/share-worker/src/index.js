...
async function snapshot() {
  ...
  const r = await fetch(u.toString(), { cf:{cacheTtl:0}, headers:{Accept:"application/json"} });
...
function findPost(s,id) {
  ...
  const title=String(manifest?.title||row?.title||"Berita SMP Negeri 1 Susukan").trim();
...
async function imageResponse(req,id) {
  try {
    ...
    if(/^https?:\/\//i.test(cover)) {
      ...
      const h=new Headers(r.headers);h.set("Cache-Control","public, max-age=3600, s-maxage=3600");h.set("Access-Control-Allow-Origin","*");
      return new Response(r.body,{status:200,headers:h});
    }
    const d=parseDataUri(cover);
    if(!d) return Response.redirect(`${SITE}gerbang.jpg`,302);
    return new Response(d.body,{status:200,headers:{"Content-Type":d.mime,"Cache-Control":"public, max-age=3600, s-maxage=3600","Access-Control-Allow-Origin":"*"}});
...
function htmlResponse(req,id,p) {
  ...
  return new Response(body,{status:200,headers:{"Content-Type":"text/html; charset=utf-8","Cache-Control":"public, max-age=60, s-maxage=300","X-Robots-Tag":"index, follow"}});
}
...
export default {
  async fetch(request) {
    ...
    if(u.pathname==="/health") return new Response(JSON.stringify({ok:true,service:"paibp-smart-share",version:"118"}),{headers:{"Content-Type":"application/json","Cache-Control":"no-store"}});
