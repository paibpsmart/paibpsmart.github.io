...
#spensus-news-v102 .sg-cover,#spensus-news-v102 .sg-thumbs{display:none!important}
...
#spensus-news-v102 .sg-story-carousel{position:relative;margin:22px 0 0;background:#031b1c;border-top:1px solid rgba(255,255,255,.08);border-bottom:1px solid rgba(255,255,255,.08)}
...
#spensus-news-v102 .sg-story-slide{position:relative;flex:0 0 100%;display:grid;place-items:center;min-width:0;scroll-snap-align:start;scroll-snap-stop:always;background:linear-gradient(145deg,#031c1d,#092b33);cursor:zoom-in}
#spensus-news-v102 .sg-story-slide img{display:block;width:100%;height:auto;max-height:min(74vh,760px);object-fit:contain;object-position:center;background:#031b1c;filter:none!important;transform:none!important;image-rendering:auto}
...
`;
  ...
  function buildCarousel(){
    ...
    if(!ensureExpanded(feature)){setTimeout(buildCarousel,20);return}
    ...
    const summary=$(".sg-author",feature)||$(".sg-summary",feature);summary?.insertAdjacentElement("afterend",wrap);
  ...
  function schedule(){[0,40,120,350].forEach(ms=>setTimeout(buildCarousel,ms))}
  ...
  const init=()=>{injectStyle();[80,240,700,1600].forEach(ms=>setTimeout(buildCarousel,ms))};
