const CACHE_NAME="paibp-smart-v106-clean-icons-r1";
const STATIC=[
  ...
  "./global-visual-v102.css?v=102","./portal-fastnav-v102.js?v=102","./class-access-v102.js?v=102","./ui-clean-v102.js?v=102","./icon-art-v85.js?v=106","./ui-final-v105.css?v=106","./ui-final-v105.js?v=106","./mapel-premium-v102.css?v=102","./mapel-card-icons-v102.js?v=102",
...
self.addEventListener("activate",event=>{event.waitUntil((async()=>{
  ...
  for(const client of clients){try{const u=new URL(client.url);if(u.origin===self.location.origin&&u.searchParams.get("ui")!=="106"){u.searchParams.set("ui","106");await client.navigate(u.href)}}catch{}}
})())});
...
const v=url.searchParams.get("v");if(v==="106"&&(request.destination==="script"||request.destination==="style")){event.respondWith(networkFirst(request));return}if(v==="102"&&(request.destination==="script"||request.destination==="style")){event.respondWith(staleWhileRevalidate(request));return}if(request.destination==="image"||request.destination==="font"){event.respondWith(cacheFirst(request));return}event.respondWith(networkFirst(request))});