const CACHE_NAME="paibp-smart-v100-fast-static";
const STATIC=[
  "./logo-spensus.png",
  ...
  "./news-editor-v99.css?v=100",
  "./news-editor-v100.js?v=100"
];
...
self.addEventListener("fetch",event=>{const request=event.request;if(request.method!=="GET")return;const url=new URL(request.url);if(url.origin!==self.location.origin)return;if(request.mode==="navigate"||request.destination==="document"){event.respondWith(networkFirst(request));return;}if(["86","87","88","89","90","92","93","94","95","96","97","98","99","100"].includes(url.searchParams.get("v"))&&(request.destination==="script"||request.destination==="style")){event.respondWith(cacheFirst(request));return;}if(request.destination==="image"||request.destination==="font"){event.respondWith(cacheFirst(request));return;}event.respondWith(networkFirst(request));});