(() => {
  ...
  function bookCard(book, index) {
    ...
    const fallback = `<div class="lit-cover-fallback-v35"><span>${escapeHtml(book.icon || (book.kind === "collection" ? categoryById[book.category]?.icon : "📕"))}</span><small>${escapeHtml(book.kind === "collection" ? "SPENSUS COLLECTION" : compact(book.source, 22))}</small><b>${escapeHtml(compact(book.title, 72))}</b></div>`;
