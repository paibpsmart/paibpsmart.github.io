(() => {
  ...
  async function compress(file,maxW=1440,quality=.78){
    ...
    c.getContext("2d",{alpha:false}).drawImage(bitmap,0,0,w,h);bitmap.close?.();
  ...
  async function addFiles(files){
    ...
    if(room<=0){photoStatus("Maksimal 10 foto untuk satu berita.","error");return}
    const selected=[...files].slice(0,room);
    if(files.length>room)photoStatus(`Maksimal 10 foto. ${selected.length} foto pertama diproses.`,"error");
    for(let i=0;i<selected.length;i++){
      const f=selected[i];
      photoStatus(`Mengoptimalkan foto ${i+1} dari ${selected.length}…`);
      try{
        const full=await compress(f,1440,.78);
        const thumb=await compress(f,560,.70);
        photos.push({id:uid(),name:f.name,full,thumb});
        renderPhotos();previewCard();
      }catch{photoStatus(`Foto ${f.name} gagal diproses.`,"error")}
    }
    photoStatus(`${photos.length} dari 10 foto siap. Foto pertama menjadi sampul.`);
  }

  function renderPhotos(){
    const root=$("#ne-media-grid"),count=$("#ne-photo-count");
    if(count)count.textContent=`${photos.length}/10 foto`;
    ...
    root.innerHTML=photos.map((p,i)=>`<article class="ne-photo-card ${i===0?'is-cover':''}" data-photo-id="${esc(p.id)}"><span class="ne-photo-badge">${i===0?'SAMPUL':`FOTO ${i+1}`}</span><img src="${esc(p.thumb)}" alt="Pratinjau foto ${i+1}"><footer><span class="ne-photo-name">${esc(p.name||`Foto ${i+1}`)}</span><div class="ne-photo-actions"><button type="button" data-photo-left="${i}" aria-label="Geser ke kiri">←</button><button type="button" data-photo-right="${i}" aria-label="Geser ke kanan">→</button><button type="button" class="is-cover-btn" data-photo-cover="${i}">${i===0?'Sampul aktif':'Jadikan sampul'}</button><button type="button" class="is-danger" data-photo-remove="${i}" aria-label="Hapus foto">×</button></div></footer></article>`).join("");
    $$('[data-photo-left]',root).forEach(b=>b.onclick=()=>movePhoto(Number(b.dataset.photoLeft),-1));
    $$('[data-photo-right]',root).forEach(b=>b.onclick=()=>movePhoto(Number(b.dataset.photoRight),1));
    $$('[data-photo-cover]',root).forEach(b=>b.onclick=()=>makeCover(Number(b.dataset.photoCover)));
    $$('[data-photo-remove]',root).forEach(b=>b.onclick=()=>removePhoto(Number(b.dataset.photoRemove)));
  }
  ...
  function previewCard(){
    const title=$("#ne-title").value.trim()||"Judul berita akan tampil di sini";
    const summary=$("#ne-summary").value.trim()||"Ringkasan berita akan muncul otomatis ketika Anda mengetik.";
    const cat=$("#ne-category").value,date=$("#ne-date").value;
    ...
    const mini=$("#ne-mini-gallery");mini.innerHTML=photos.slice(1,5).map(p=>`<img src="${esc(p.thumb)}" alt="Foto tambahan">`).join("");mini.hidden=photos.length<2;
  }
  ...
  function formItem(statusName="draft"){
    ...
    if(!title||!summary||!content||!date)throw new Error("Judul, tanggal, ringkasan, dan isi berita wajib diisi.");
    if(!photos.length)throw new Error("Tambahkan minimal satu foto sebelum pratinjau dan publikasi.");
    const now=new Date().toISOString();
    return{id,slug:slug(title),title,summary,content,date,category,year:Number(date.slice(0,4)),month:Number(date.slice(5,7)),thumbnail:photos[0].thumb,media:photos.map((p,i)=>({src:p.full,thumbnail:p.thumb,name:p.name,order:i+1,cover:i===0})),status:statusName,author:"Sunarso, S.Pd.I, Gr",updatedAt:now,createdAt:now};
  }

  async function saveDraft(){
    try{const item=formItem("draft");const old=(await allLocal()).find(x=>x.id===item.id);if(old)item.createdAt=old.createdAt;await putLocal(item);$("#ne-id").value=item.id;$("#ne-editor-status").textContent="Draft tersimpan";status("Draft tersimpan aman di perangkat ini.","ok");refreshLists()}catch(e){status(e.message||"Draft gagal disimpan.","error")}
  }
  $("#ne-save-draft").addEventListener("click",saveDraft);

  function articleGalleryHtml(item){
    const list=item.media||[];if(!list.length)return"";
    const shown=list.slice(0,3),cls=list.length===1?"count-1":list.length===2?"count-2":list.length===3?"count-3":"count-many";
    return `<div class="ne-gallery-layout ${cls}">${shown.map((p,i)=>`<figure><img src="${esc(p.src)}" alt="${esc(item.title)} — foto ${i+1}">${i===shown.length-1&&list.length>shown.length?`<span class="ne-gallery-more">+${list.length-shown.length}</span>`:""}</figure>`).join("")}</div>`;
  }
  function contentHtml(text){return String(text||"").split(/\n\s*\n|\n/).map(x=>x.trim()).filter(Boolean).map(p=>`<p>${esc(p)}</p>`).join("")}
  function openPreview(){
    let item;try{item=formItem("preview")}catch(e){status(e.message,"error");return}
    pendingPublish=item;
    $("#ne-article-preview").innerHTML=`<article class="ne-article-preview"><header class="ne-article-hero"><div class="ne-article-meta"><span>${esc(item.category)}</span><time>${esc(formatDate(item.date))}</time><span>${item.media.length} foto</span></div><h1>${esc(item.title)}</h1><p class="ne-article-lead">${esc(item.summary)}</p></header><section class="ne-article-gallery">${articleGalleryHtml(item)}</section><section class="ne-article-content">${contentHtml(item.content)}</section></article>`;
    $("#ne-preview-modal").hidden=false;document.body.style.overflow="hidden";status("Pratinjau dibuka. Periksa isi dan foto sebelum menekan Terbitkan Sekarang.","ok")
  }
  function closePreview(){$("#ne-preview-modal").hidden=true;document.body.style.overflow="";pendingPublish=null}
  $("#ne-open-preview").addEventListener("click",openPreview);
  $("#ne-form").addEventListener("submit",e=>{e.preventDefault();openPreview()});
  $$('[data-ne-close-preview]').forEach(b=>b.addEventListener('click',closePreview));
  document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!$("#ne-preview-modal").hidden)closePreview()});

  async function apiPost(action,data){
    const ctl=new AbortController(),t=setTimeout(()=>ctl.abort(),12000);
    try{
      const r=await fetch(ENDPOINT,{method:"POST",mode:"cors",cache:"no-store",headers:{"Content-Type":"text/plain;charset=UTF-8","Accept":"application/json"},body:JSON.stringify({app:"paibp-smart",version:"99",action,data,origin:location.origin}),signal:ctl.signal});
      const text=await r.text();let j={};try{j=JSON.parse(text)}catch{}
      if(!r.ok||j.ok!==true){const raw=String(j.error||j.message||"");if(/aksi\s*post|tidak\s*dikenal|unknown|unsupported/i.test(raw))throw new Error("Server publikasi berita belum diaktifkan untuk fitur Editor Berita. Draft tetap aman; konfigurasi Worker perlu diperbarui sebelum berita dapat tayang ke semua perangkat.");throw new Error(raw||`Server belum mengonfirmasi publikasi (HTTP ${r.status}).`)}
      return j;
    }catch(e){if(e?.name==='AbortError')throw new Error('Server tidak merespons dalam 12 detik. Draft tetap aman.');throw e}finally{clearTimeout(t)}
  }
  ...
  $("#ne-publish-confirm").addEventListener("click",async()=>{
    ...
    const button=$("#ne-publish-confirm");button.disabled=true;button.textContent="Menerbitkan…";
    let item={...pendingPublish,status:"pending"};
    try{
      const old=(await allLocal()).find(x=>x.id===item.id);if(old)item.createdAt=old.createdAt;await putLocal(item);
      const result=await apiPost("newsPublishV96",item);
      item.status="published";item.publishedAt=result.publishedAt||new Date().toISOString();item.publicUrl=result.url||result.publicUrl||"";item.serverId=result.articleId||result.id||item.id;await putLocal(item);
      $("#ne-id").value=item.id;$("#ne-editor-status").textContent="Tayang";status("Berita dikonfirmasi server dan sudah berstatus Tayang.","ok");closePreview();remote=await apiList();refreshLists();
    }catch(err){item.status="draft";await putLocal(item);$("#ne-editor-status").textContent="Draft";status(err.message||"Publikasi belum berhasil. Draft tetap aman di perangkat.","error");closePreview()}
  ...
  function normalizeStoredMedia(x){
    if(Array.isArray(x?.media)&&x.media.length)return x.media.slice(0,MAX_PHOTOS).map((m,i)=>({id:uid(),name:m.name||`Foto ${i+1}`,full:m.src||m.full||"",thumb:m.thumbnail||m.thumb||m.src||""})).filter(p=>p.full||p.thumb);
    if(x?.thumbnail)return[{id:uid(),name:"Foto sampul",full:x.media?.[0]?.src||x.thumbnail,thumb:x.thumbnail}];
    return[];
  }
